...
$(function() {
    ...
    $('#Btn1').on('click', function() {
        ...
        if(! $('.photo').hasClass('photo--empty')){
            ...
            $.ajax({
                url:URL.revokeObjectURL('/products/image/1'),
...
alert(err);
                ...
                complete:function(){
                    ...
                    $('#message').remove();
                    $('#con-message').append('<div class="alert alert-info" id="message">\n' +
            ...
            $('#message').remove();
            window.location.reload(true);
            $('#con-message').append('<div class="alert alert-danger" id="message">\n' +
                '                                    <button type="button" class="close" data-dismiss="alert">\n' +
                '                                        <i class="ace-icon fa fa-times"></i>\n' +
                '                                    </button>\n' +
                ' لطفا فایل تصویر مورد نظر را انتخاب کنید ' +
                '                                    <br>\n' +
                '                                </div>'
            );
    ...
    $('#Btn2').on('click', function() {
        ...
        if(! $('.photo').hasClass('photo--empty')){
            ...
            // Submit Form and upload file
            $.ajax({
                url:URL.revokeObjectURL('/products/image/2'),
                data: fd,// the formData function is available in almost all new browsers.
                type:"POST",
                contentType:false,
                processData:false,
    ...
    $('#Btn3').on('click', function() {
        ...
        if(! $('.photo').hasClass('photo--empty')){
            ...
            // Submit Form and upload file
            $.ajax({
                url:URL.revokeObjectURL('/products/image/3'),
                data: fd,// the formData function is available in almost all new browsers.
                type:"POST",
                contentType:false,
                processData:false,
    ...
    $('#Btn4').on('click', function() {
        ...
        if(! $('.photo').hasClass('photo--empty')){
            ...
            // Submit Form and upload file
            $.ajax({
                url:URL.revokeObjectURL('/products/image/4'),
                data: fd,// the formData function is available in almost all new browsers.
                type:"POST",
                contentType:false,
                processData:false,
