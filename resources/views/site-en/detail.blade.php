            ...
            <div itemscope itemtype="http://schema.org/Product">
                ...
                <div class="row product-info">
                    <div class="col-sm-6">
                        ...
                        <div class="center-block text-center"><span class="zoom-gallery"><i class="fa fa-search"></i>Click on the image to view the gallery.</span>
                        </div>
                    ...
                    <div class="col-sm-6">
                        <ul class="list-unstyled description">
                            ...
                            <li><b>Purchase Points :</b> {{ $product->rate }}</li>
                            <li><b>Count :</b> <span class="instock">{{ $product->count }}</span></li>
                        </ul>
                        <ul class="price-box">
                            <li class="price" itemprop="offers" itemscope itemtype="http://schema.org/Offer"><span
                                        class="price-old">{{  ($product->price)+10   }} Toman</span> <span
                                        itemprop="price">{{ $product->price }}  Toman<span itemprop="availability"
                                                                                          content="existent"></span></span>
                            </li>
                            <li></li>
                            <li>No tax : {{  ($product->price)-3   }} Toman</li>
                        </ul>
                        <div id="product">
                            <h3 class="subtitle">Available choices</h3>
                ...
                <div class="owl-carousel related_pro">
                    @foreach($products as $product)
                        <div class="product-thumb">
                            <div class="image"><a href="{{ route('en.detail', $product->id) }}"><img src="{{ $product->image1 }}"
                                                                                                  alt="{{ $product->name }}" title="{{ $product->name }}"
                                                                                                  class="img-responsive"/></a></div>
                            <div class="caption">
                                <h4><a href="{{ route('en.detail', $product->id) }}">{{ $product->name }}</a></h4>
                                <p class="price"><span class="price-new">{{ $product->price }} Toman</span> <span
                                            class="price-old">{{ ($product->price)-3 }} Toman</span>
            ...
            <div class="side-item">
                @foreach($products as $product)
                    <div class="product-thumb clearfix">
                        <div class="image"><a href="{{ route('en.detail', $product->id) }}"><img src="{{ $product->image2 }}"
                                                                                              alt="{{ $product->name }}"
                                                                                              title="{{ $product->name }}"
                                                                                              class="img-responsive"/></a></div>
                        <div class="caption">
